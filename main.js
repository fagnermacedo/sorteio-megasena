// Preços Oficiais da Caixa Econômica Federal
...
window.onload = function() {
    ...
    obterTotalVisitas(); // Lê o JSON que o Workflow atualiza
};
...
function atualizarDataModificacao() {
    ...
    document.getElementById('dataUpdate').innerText = `Atualizado dia ${data.toLocaleDateString('pt-BR', opcoes)}`;
}

async function obterTotalVisitas() {
    try {
        // Busca o valor atualizado do seu arquivo acessos.json no GitHub
        ...
        document.getElementById('valorVisitas').innerText = "1";
    }
}
...
function calcularCusto() {
    ...
    if (!PRECO_UNITARIO[dezenas]) {
        document.getElementById('resultadoCusto').innerHTML = "Escolha entre 6 e 20 números.";
        return;
    }
...
function sortearNumeros() {
    ...
    for (let i = 1; i <= totalJogos; i++) {
        ...
        const lista = Array.from(numeros).sort((a, b) => a - b).map(n => n.toString().padStart(2, '0'));
        exibirLinha(i, lista);
    }
}

function exibirLinha(index, dezenas) {
    const grid = document.getElementById('gridSorteio');
